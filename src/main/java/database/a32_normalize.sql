...
3. 정규화와 이상현상
	1) 이상 현상
		불필요한 *데이터중복으로 인해 릴레이션에 대한 데이터 삽입,수정,삭제 연산을 수행할 때, 발생할 수 있는
		부작용을 말한다.
	2) 정규화 과정
...
5. 함수 종속성
	1) 속성들 간의 관련성
		ex) 컬럼간의 관련성을 의미한다.
	cf) 정규화란 함수적 종속성을 이용하여, 릴레이션(테이블)을 연관성이 있는 속
		성(컬럼)들로만 구성되도록 분해하여 이상 현상이 발생하지 않는(무결성 있는 데이터) 
		올바른 릴레이션(테이블)으로 만들어 나가는 과정이다.		 
	2) 함수 종속
		"X가 Y를 함수적으로 결정한다"
		X -> Y 로 표현(X는 결정자, Y는 종속자)
		보통 X를 테이블의 key로 만들어고, Y는 그 구성요소들을 만한다.
		ex) 사원번호 -> (사원명, 직책, 입사일, 급여, 보너스)
		사원번호 -> 사원명, 사원번호 -> 직책	
	3) 함수 종속 관계 판단시, 유의 사항
		속성 자체의 특성과 의미를 기반으로 함수 종속성을 판단해야 함..
			- 속성 값은 계속 변할 수 있으므로 현재 릴레이션(테이블)에 포함된 속성(컬럼)의 값
			만으로 판단하면 안 됨..
		일반적으로 기본키(사원번호)와 후보키(주민번호)는 릴레이션(테이블)의 다른 모든 속성(컬럼)
			들을 함수적으로 결정한다.(결정자)
		기본키(사원번호)나 후보키(주민번호)가 아니어도 다른 속성 값을 유일하게 결정하는 속성은
	...
	4) 완전 함수 종속
		...
		ex) 당첨여부는 키인{고객아이디, 이벤트번호}에 완전 함수 종속됨..
	5) 부분 함수 종속
		릴레이션(테이블)에서 속성집합(컬럼1의 데이터) Y가 속성집합(컬럼2의 데이터) X의 
		전체가 아닌 일부분에서 함수적으로 종속됨을 의미
		ex) 고객이름은 {고객 아이디, 이벤트번호}에 부분 함수 종속된다.	
	# 고객 아이디 -> 고객이름   : 고객이름은 고객아이디에 완전 종속적
	  {고객아이디, 이벤트번호} -> 당첨여부 : 담첨여부는 고객아이디와 이벤트번호(복합키)에 완전종속
	  {고객아이디, 이벤트번호} -> 고객이름 : 고객이름은 고객아이디와 이벤트번호(복합키)에 부분종속
...
6. 정규화
	...
	2) 주의 사항
		정규화를 통해 릴레이션(테이블)은 무손실 분해 되어야 한다.
		- 릴레이션의 의미상 동등한 릴레이션(테이블)들로 분해되어야 한고, 분해로 인한 정보 손실이 발생하지 않아야 함.
		- 분해된 릴레이션(테이블)들을 자연 조인(where 분해후테이블.컬럼 = 분해후 테이블.컬럼)하면 
		  분해 전의 릴레이션(테이블)으로 복원 가능해야 함..
	3) 정규형(NF:Normal form)
		릴레이션(테이블)이 정규화(분해)된 정도
		각 정규형마다 제약조건이 존재
			- 정규형의 차수가 높아질수록 요구되는 제약조건이 많아지고 엄격해짐
		릴레이션(테이블)의 특성을 고려해서 적합한 정규형으로 나아가 진다.
	4) 정규화 과정
		기본 정규형 = 제1 정규형, 제2 정규형, 제3 정규형, 보이스/코드 정규형(BCNF)		  
		고급 정규형 = 제4 정규형, 제5 정규형
		상위 정규가 끝나야지 하위 정규화 과정을 진행할 수 있다.
			제1 정규형 > 제2 정규형 > 제3 정규형 > 보이스/코드 정규형(BCNF) > 제4 정규형 > 제5 정규형		  	  
				
7. 제1정규화
8. 제2정규화
9. 제3정규화
	1) 이행적 함수 종속성
		릴레이션(테이블)을 구성하는 3개의 속성(컬럼) 집합 X, Y, Z에 대해 함수 종속 관계가
		X -> Y, Y->Z가 존재하면 논리적으로 X->Z가 성립되는데, 이 때 Z가 X에 이행적으로 함수 종속되었다고 함.
	2) 개념
		릴레이션(테이블)이 제 2정규형에 속하고, 기본키가 아닌 모든 속성이 기본 키에 이행적 함수 종속이 되지 않으면
			제 3정규형을 만족함
		제2정규형에 속하는 릴레이션(테이블)이 제3정규형을 만족하게 하려면
			- 모든 속성이 키본키에 이행적 함수 종속이 되지 않도록 분행..
 **/
...
AS SELECT cu_id, grade FROM normal_exp03;
...
식별자 관계 : 
	...
	ex) 고객이 물건하나의 주문을 사용할 때.
	컬럼을 최소로 사용할 수 있지만, 테이블간에 연관관계가 많을 때,
	복합키를 많이 만들어야 되면, 복잡성은 높아질 때가 많다.
	

비식별자 관계 : 
	참조하는 테이블의 키를 현재 테이블에 키로 사용하지 않을 때.
	ex) emp dept 관계
	pk 컬럼을 따로 잡아, 컬럼이 추가 되는 경향이 있지만,
...
10. 보이스/코드 정규형(BCNF:Boyce/Codd Normal Form)
	1) 필요성
		하나의 릴레이션(테이블)에 여러 개의 후보키(사원테이블에 주민번호, 복합된 후보키)
		가 존재하는 경우, 제 3정규형까지 모두 만족해도 이상 현상이 발생할 수 있다.
 * 
 * */
...
INSERT INTO NORMALFORM3 values('oarange','기초토익','P004');
SELECT * FROM NORMALFORM3;
-- ~ 16:10(캡쳐,신호전송) 보이스코드 정규화를 처리해서 제출하세요..  2조
CREATE TABLE NORMALFORM_boyce_1
as
SELECT cus_id, tech_id FROM NORMALFORM3;
CREATE TABLE NORMALFORM_boyce_2
as
SELECT DISTINCT tech_id, ilesson  
FROM NORMALFORM3
ORDER BY tech_id;
...
함수 종속이 아닌 다치 종속(MVD; Multi Valued Dependency)을 제거하면
제 4 정규형에 속함

제 5 정규형
릴레이션이 제 4 정규형을 만족하면서, 
후보키를 통하지 않는 조인 종속 (JD; Join Dependency)을 제거하면 
제 5 정규형에 속함
# 정규화 시 주의 사항
모든 릴레이션이 제 5 정규형에 속해야만 바람직한 것은 아님
일반적으로 제 3 정규형이나 보이스/코드 정규형에 속하도록 
릴레이션을 분해하여 데이터 중복을 줄이고 이상 현상을 해결하는 경우가 많음

*** 결국, 정규화 과정은 *원자성이라는 개념에 의해서, 원인에 대한 분석으로
1~5정규 및 보이스코드 정규를 처리하기에 식별자와 비식별자를 통해 효과적으로 이상현상(삽입/삭제/수정)
이 발생하지 않도록 테이블을 분해시켜가능 과정으로 진행하면 된다.
